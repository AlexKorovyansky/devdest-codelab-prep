...
Поздравляем! Вы уже сделали уникальное решение и научились работать с IoT и Firebase! Теперь давайте сделаем веб-страницу, отображающую «уровень здоровья» и позволяющую менять знанение параметров time_to_discharge и time_to_recharge. 

Для этого нам нужно сверстать страницую, обновить настройки Firebase приложения и реализовать взаимодействие с Firebase средствами Javascript. 

С целью экономия времени предлагаем взять верстку страницы можете взять по [ссылке](https://drive.google.com/a/handsome.is/file/d/0B_f_gRIwcIw5WTdHYUFjMUFCNGM/view?usp=sharing). В ней нас интересует три элемента:
