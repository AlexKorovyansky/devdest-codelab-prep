...
Читая дальнейший текст README, вы шаг за шагом реализуете:
...
Если на данный момент, вы никогда всерьез не работали с Arduino или Firebase — не волнуйтесь, потому что во-первых мы начнем с самых простых вещей и будем очень плавно двигаться вперед, а во-вторых WioLink и Firebase это очень простые и быстрые в использования решения. Код лаб рассчитан на 60-90 минут. Если вы справитесь быстрее, мы будем рады увидеть ваши предложения по во

Созданное решение открывает ... в каждый момент времени мы знаем "уровень здоровья" можем считать статистику и реализовывать дополнительные фичи на ее основе. Например, если пользователь очень долго сидит в "красной" зоне (когда уровень здоровья равен нулю), мы можем автоматически отправить SOS-сообщение его другу/коллеге и за дружеской заботы помочь передохнуть и размяться. Также можно считать среднюю недельную статистику для команды (если вся команда использует умные офисные кресла) — готовы поспорить, что статистика большинства команд будет за пределами "зеленой зоны" (обозначающий здоровый график перерывов и разминок). Выход из "красной" или "желтой" зоны в "зеленую" это отличный челендж для любой команды, которая заботится о здоровье. 
